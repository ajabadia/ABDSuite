...
if (fs.existsSync(pkgPath)) {
  try {
    ...
    isLibrary = pkg.name === '@ajabadia/styles' || pkg.name === '@ajabadia/satellite-sdk' || pkg.name === '@ajabadia/ecosystem-widgets' || pkg.name.endsWith('-sdk') || pkg.name.endsWith('-widgets');
  } catch (e) {}
}
...
try {
  ...
  allFiles.forEach((filePath, index) => {
    ...
    if (ext === '.ts' || ext === '.tsx') {
      // Structural length and inline styles
      ...
      // i18n coverage (only for applications, skipped for pure library)
      if (!isLibrary && (phase === 'all' || phase === 'i18n') && ext === '.tsx') {
        lines.forEach((line, idx) => {
      ...
      if (phase === 'all' || phase === 'purity') {
        lines.forEach((line, idx) => {
          if (line.includes(': any') || line.includes('as any')) {
            if (!relPath.includes('scripts/') && !line.includes('//') && !line.includes('unknown as any') && !line.includes('config: unknown')) {
  ...
  if (!isLibrary && !hasIndustrialCoreImport && (phase === 'all' || phase === 'structural')) {
    addFinding('HIGH', 'FIRE:ABD_STYLES_MISSING', 'globals.css', 1, 'The project does not import "@ajabadia/styles/dist/styles/industrial-core.css" in any CSS file.');
  }
