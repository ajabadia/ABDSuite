...
| **`ABDtenantGovernance`** | `SYS_CERTIFIED` | Vercel (`abd-tenant-governance.vercel.app`) | Operativo (Control Plane) |
...
*   [ ] **1.1 Delegación de DNS en GoDaddy**: Configurar los registros DNS de tu dominio para enlazar subdominios a los proyectos de Vercel.
    ...
    *   `gobernanza.tudominio.com` -> `ABDtenantGovernance`
...
*   [x] **1.4 Firma y Validación Criptográfica de la Sesión**: Adaptar las aplicaciones satélite (`ABDQuiz`, `ABDtenantGovernance`) para que la cookie `abd_session` almacene directamente el JWT firmado por `ABDAuth` y validar su firma usando `AUTH_JWT_SECRET` en `getIndustrialSession()`.
...
*   [x] **4.3 Migración de Ingestores**: Actualizar `ABDtenantGovernance` y `ABDQuiz` para que envíen sus logs asíncronos mediante peticiones HTTP `fetch` a `ABDLogs` en lugar de conexiones directas de base de datos.
...
*   [x] **8.1 Motor de Evaluación ABAC (Guardian Engine)**: Importación y adaptación del motor de permisos `GuardianEngine` dentro de `ABDtenantGovernance`. Despliegue de endpoint interno `POST /api/internal/guardian/evaluate` para ser consumido de manera federada por cualquier satélite.
...
*   [x] **9.3 Motor de Búsqueda Integrado (MongoDB Atlas Search)**: Implementado endpoint `GET /api/v1/documents/search` en ABDFiles con pipeline `$search` sobre `title` y `tags`, filtrado por `tenantId`, con paginación y fuzzy matching. Se elige Atlas Search sobre Elasticsearch/OpenSearch por no requerir infraestructura adicional.
...
*   [x] **9.6 Versionado Dinámico de Esquemas de Eventos**: Diseñar un adaptador/registrador de esquemas para permitir la evolución fluida de los formatos de mensajes sin interrumpir servicios legacy.
...
*   [x] **9.8 Integración OAuth2 en `ABDAuth` para Storage Externo**: Implementados los providers reales `GoogleDriveProvider` (Service Account) y `OneDriveProvider` (Azure AD Client Credentials / Microsoft Graph API) en `ABDFiles`. Actualizados los templates de credenciales en `ABDtenantGovernance`. El tenant (no el usuario) decide qué proveedor activar desde el panel de administración.
*   [ ] **9.9 Gobernanza y Migraciones de Almacenamiento**: Revisar en `ABDtenantGovernance` la configuración del conector activo por Tenant. Investigar políticas de migración automática de archivos entre proveedores y soporte para múltiples conectores activos según la sensibilidad del documento.
...
*   [x] **9.13 Migración de Satélites a Sub-Path Imports**: Refactorizar progresivamente todos los satélites (`ABDAuth`, `ABDQuiz`, `ABDtenantGovernance`, etc.) para importar desde los puntos de entrada específicos de `@ajabadia/satellite-sdk` (ej. `/db`, `/logger`) en lugar de usar el barrel index general. *(Completado: ~244 archivos migrados en 9 satélites, con 10 sub-paths expuestos).*
*   [x] **9.14 Cobertura ABAC Extendida**: Integrar ABAC en las rutas de API críticas de toda la suite.
     *   [x] **ABDFiles**: abac.ts con `evaluateAccess` del SDK.
     *   [x] **ABDQuiz**: abac.ts + `ensureQuizAccess.ts` con `evaluateAccess`.
     *   [x] **ABDAuth**: ABAC integrado con `assertAccess()` en 4 rutas admin críticas (users, applications, applications/[id], tenants).
...
*   **Verificación de Estilos Unificados**: Se comprobó que `ABDAuth`, `ABDtenantGovernance` and `ABDQuiz` importan e integran el núcleo visual de `@abd/styles` (`industrial-core.css`) bajo Tailwind CSS v4.
*   **Resolución de Desviaciones**:
    *   Se identificó un fallo en la fase de Pureza (4/6) y Calidad de Código (6/6) en `ABDtenantGovernance` debido a un casteo con `any` (`LinkComponent={Link as any}`).
...
*   **Patrón Delegador en Satélites**: Se simplificaron los scripts locales de `ABDAuth`, `ABDtenantGovernance` y `ABDQuiz` para que apunten al script unificado de `node_modules/@abd/styles`.
...
*   **Licenciamiento y Handshake de Rigor**: Implementación de controles estrictos de `allowedApps` en `ABDAuth` y `ABDtenantGovernance`, garantizando la restricción del Super Admin en tenants sin licencia.
...
*   **Enlace de Desarrollo y Symlink**: Vinculación local de `@abd/styles` (`file:../ABDStyles`) en `package.json` de `ABDtenantGovernance` para desarrollo interactivo inmediato.
...
*   **Corrección de Analizador de Componentes UI**: Se diagnosticó que `ABDtenantGovernance` estaba purgando erróneamente las clases utilitarias dinámicas del menú lateral (`-translate-x-full`). Se solucionó inyectando `@source "../../node_modules/@abd/styles/dist/**/*.js";` en el `globals.css` central.
*   **Alineamiento del Shell de Componentes**: Modificación de las pantallas base de `ABDtenantGovernance` (`page.tsx` y `admin/page.tsx`) integrándolas bajo el mismo estándar de espaciado dinámico flotante que `ABDAuth` (incluyendo `pt-24 pb-12` y `relative z-10`), erradicando sobre-posicionamientos con los gatillos de configuración y la hamburguesa del menú.
...
*   **Flujo de Invitación Aséptico**: Creación de modal en `ABDtenantGovernance` para invitar usuarios definiendo su rol y apps permitidas. Envío de emails transaccionales mediante Resend (simulado en dev).
...
*   **Auditoría Industrial**: Resolución de advertencias A11y, TSC, `any` e I18n alcanzando el certificado de pureza en `ABDtenantGovernance`.
...
*   **Eliminación de Logins Locales redundantes**: Ocultación del botón de login en el panel de configuración de los satélites (`ABDQuiz`, `ABDLogs`, `ABDtenantGovernance`) para canalizar toda autenticación a través del portal centralizado `ABDAuth`.
...
Anteriormente, la selección del tenant activo se repetía en múltiples páginas de `ABDtenantGovernance` (usuarios, permisos, espacios, etc.) mediante desplegables locales ad-hoc, violando el principio DRY y creando inconsistencias en la experiencia de usuario. No existía un componente centralizado que filtrara el contexto de tenant de manera global.

**Solución Implementada:**
Se ha desarrollado un componente unificado `TenantSelector` dentro de la librería compartida `@abd/styles` e integrado globalmente en la barra de controles flotante (`fixed top-6 right-6`) de los layouts principales en `ABDtenantGovernance` y `ABDLogs`.
...
*   **Propagación de Refactorización**: Despliegue e implementación de las mejoras y tipados en el `TenantSelector` a través de todos los satélites (`ABDQuiz`, `ABDLogs`, `ABDtenantGovernance`).
...
*   **Certificación Industrial Uniforme**: Verificación exitosa de los scripts `pnpm run build` y `./scripts/abd-audit.ps1` en los cuatro repositorios principales (`ABDAuth`, `ABDQuiz`, `ABDLogs`, `ABDtenantGovernance`), logrando la certificación `ERA 11 COMPLIANT` libre de advertencias en todas las fases de compilación y empaquetado.

### Sesión 15: 22 de Mayo de 2026 (Refinamiento de Permisos y Gobernanza Polimórfica en Espacios)
*   **Arquitectura de Espacios Polimórficos**: Modificación profunda del esquema de Espacios (`SpaceSchema`) en ABDtenantGovernance para integrar un modelo Many-to-Many polimórfico a través del array `collaborators`, enlazando entidades `USER` y `GROUP` para delegación de permisos sin acoplamiento duro.
...
*   **Motor ABAC Transversal (GuardianEngine)**: Implementación de la lógica pura del motor `GuardianEngine` en `ABDtenantGovernance` con el endpoint federado `/api/internal/guardian/evaluate`.
...
*   **Auditoría y Certificación Total (ERA-11)**: Subsanación profunda de violaciones de linting, tipos `any` genéricos, errores de internacionalización y advertencias del compilador TSC en todos los proyectos (`ABDAuth`, `ABDQuiz`, `ABDLogs` y `ABDtenantGovernance`), logrando conformidad de 100% de calidad.

### Sesión 17: 22 de Mayo de 2026 (Refactorización Estética y Aceptación de la Gobernanza Visual)
*   **Limpieza Técnica (Fire Rules)**: Refactorización masiva en `ABDtenantGovernance` reduciendo archivos monolíticos (como `permissions/page.tsx` de 467 líneas a 183 líneas mediante la abstracción `GroupTreeView` y `PoliciesTable`). Todos los componentes de gobernanza ahora cumplen la norma estricta de `< 150 líneas`.
...
*   **Integración de Satélites**: Refactorizados los archivos `logs-client.ts` de `ABDtenantGovernance`, `ABDQuiz` y `ABDAuth` para delegar automáticamente en el logger del SDK, garantizando el enmascaramiento de PII en todo el ecosistema y resolviendo vulnerabilidades de fugas operativas.
...
*   **Modal de Gobernanza UI**: Creación del componente `ManageSpaceAssetsModal` en `ABDtenantGovernance` con soporte para vinculación interactiva, detección de enlaces huérfanos ("zombies") y recolección de basura.
...
*   **Hallazgos Críticos**: `ABDLogs/PROGRESS.md` y `ABDLogs/ROADMAP.md` eran copias literales de `ABDtenantGovernance`. No contenían nada específico de `ABDLogs`.
*   **Hallazgos Parciales**: `ABDAuth/ROADMAP.md`, `ABDStyles/ROADMAP.md`, `ABDAnalytics/ROADMAP.md`, `ABDtenantGovernance/ROADMAP.md`, `SECURITY_AUDIT.md` y `navbar_brainstorming.md` tenían referencias a tareas ya completadas o estados desactualizados.
...
*   **Aclaración Arquitectural**: El proveedor de storage lo elige el **tenant** (no el usuario individual) desde el panel de `ABDtenantGovernance`. Un único conector activo por tenant en cada momento.
...
*   **`ABDtenantGovernance`**: Actualizados los templates de credenciales de `googleDrive` y `oneDrive` con los campos reales. Badge actualizado a `OAuth2 / Graph API`.
...
*   **Migración masiva**: Actualizados los `package.json` de todos los satélites y librerías (`ABDAnalytics`, `ABDAuth`, `ABDFiles`, `ABDLanding`, `ABDLogs`, `ABDQuiz`, `ABDtenantGovernance`, `ABD___BASE`, `ABDSatelliteSDK`, `ABDStyles`, `ABDEcosystemWidgets`) para reemplazar versiones hardcoded por `catalog:` en las ~30 dependencias del catálogo.
...
*   **Código Muerto**: Verificación y confirmación de que no hay archivos muertos adicionales en `ABDQuiz` ni `ABDtenantGovernance`. El único candidato (`proxy.ts` en ABDtenantGovernance) se confirmó como el middleware activo de Next.js 16.
...
*   **Infraestructura de Logging Unificada**: Integración global de `logger.audit` en los bloques `catch` de errores y rutas críticas de éxito en todos los satélites (31 en `ABDQuiz`, 22 en `ABDFiles`, 19 en `ABDAuth`, y 89 en `ABDtenantGovernance`), enlazados al microservicio central `ABDLogs`.
...
*   **Desacoplamiento de Base de Datos Compartida (Fases A1 y A2)**:
    - Eliminado el antipatrón de acceso directo a la DB de `Space` en `ABDQuiz`, delegando la consulta a una API interna en `ABDtenantGovernance` con caching en Redis.
...
*   **Hito 9.13 — Migración Global a Sub-Path Imports**: Migrados exitosamente ~244 archivos fuente en los 9 satélites (`ABDQuiz`, `ABDLogs`, `ABDFiles`, `ABDAnalytics`, `ABDLanding`, `ABD___BASE`, `ABDEcosystemWidgets`, `ABDtenantGovernance` y `ABDAuth`) para importar de sub-paths dedicados en lugar del barrel general, pasando la compilación global y la suite de tests unitarios.
...
*   **Gobernanza y Orquestación GDPR**: Refactorizado `gdpr-export.ts` y el endpoint `/api/admin/gdpr/user-export` en `ABDtenantGovernance` para orquestar consultas asíncronas concurrentes a los 4 satélites (`ABDAuth`, `ABDQuiz`, `ABDFiles`, `ABDLogs`), agregando todos los archivos ZIP en un único paquete unificado descargable desde la interfaz de administración `GdprConsole.tsx`.
...
*   **Verificación**: Cero errores de compilación/Typecheck en todos los satélites (SDK, Widgets, Styles, ABDAuth, ABDFiles, ABDLogs, ABDQuiz, Governance, BASE), pasando la suite de pruebas local.
...
| ABDLogs | 126 | 67 | -59 |
| ABDQuiz | 122 | 71 | -51 |
| ABDtenantGovernance | 170 | 127 | -43 |
...
### Sesión 47: 30 de Junio de 2026 (Health Check, Motor de Búsqueda Atlas Search y ABAC en ABDAuth — Hitos 5.3, 5.4, 9.3, 9.14)

*   **ABDFiles 5.3 — Health Check**: Creado endpoint `GET /api/v1/health` en `ABDFiles` siguiendo el patrón de ABDLogs (sin auth, devuelve `{ status, timestamp, service, uptime }`). Typecheck OK.
*   **ABDFiles 5.4 — Motor de Búsqueda (MongoDB Atlas Search)**: Creado endpoint `GET /api/v1/documents/search?q=...` con pipeline `$search` sobre `title` y `tags`, filtrado por `tenantId`, paginación y fuzzy matching. Se elige Atlas Search sobre Elasticsearch/OpenSearch por no requerir infraestructura adicional. Requiere crear el índice `documents_search` en Atlas UI.
*   **ABDAuth ABAC (Hito 9.14)**: Creado `src/lib/abac.ts` con `assertAccess()` idéntico al patrón de ABDFiles. Aplicado en 4 rutas API críticas: `users/route.ts` (GET, POST, PUT), `applications/route.ts` (GET, POST), `applications/[id]/route.ts` (PATCH, DELETE), `tenants/route.ts` (GET, POST). Typecheck OK.
*   **ABDAuth Dashboard → AppShellLayout**: Migrado `[locale]/dashboard/layout.tsx` quitando `<main>` redundante (ya lo proporciona el layout padre) y simplificando a contenedor `max-w-7xl` con `GlobalFooter`.
*   **ROADMAP.md actualizado**: Hitos 9.3 y 9.14 marcados como completados. Pendiente de ABDAuth dashboard y `getServerSession` migrados según análisis de viabilidad.

### 📋 Pendientes de la Sesión 47 — Por Implementar

*   [ ] **Crear índice `documents_search` en MongoDB Atlas**: Desde la UI de Atlas, crear un Atlas Search index sobre la colección `documents` con campos `title` (autocomplete), `tags` (autocomplete) y `tenantId` (filter).
...
*   [ ] **Verificar typecheck global**: Ejecutar `pnpm run typecheck` para confirmar que todos los cambios compilan sin errores.
...
*   [ ] **Verificación E2E del refactoring de sidebar y shell layout**: Ejecutar la suite `ABDE2E` para confirmar que no hay regresiones visuales o funcionales tras la centralización de sidebar y layout shell.
*   [x] **Migrar ABDAuth a los patrones compartidos (Parcial)**:
    *   [x] **AppShellLayout**: Importado y usado en `[locale]/layout.tsx`.
    *   [x] **AppSidebarNavigation**: Usado dentro del wrapper local `SidebarNavigation.tsx`.
    *   [x] **GlobalFooter**: Importado de `@ajabadia/ecosystem-widgets`.
    *   [x] **Dashboard layout**: Migrado a usar `AppShellLayout` del layout padre — eliminado `<main>` redundante, simplificado a contenedor `max-w-7xl` con `GlobalFooter`.
    *   [ ] **getServerSession → getIndustrialSession**: 58 ocurrencias locales vs 2 del SDK. Pendiente migrar.

### Sesión 48: 6 de Julio de 2026 (Refactorización de Usabilidad, E2E y Barrels en ABDQuiz)

*   **Mejoras de Usabilidad en Formularios**: Modificado `ExamConfigForm.tsx` para detectar campos `:invalid` y desplazar el scroll/foco en submit automáticamente.
*   **Linter de Internacionalización en CI/CD**: Añadido `"lint:i18n"` en `package.json` de la raíz del monorepo e integrado en la tubería de GitHub Actions `test.yml`.
*   **Skeletons de Carga Reutilizables**: Creado componente `Skeleton` genérico en `ABDQuiz/src/components/ui/skeleton.tsx` y contenedor `loading.tsx` para `/admin/assignments` con cuadrículas fantasma animadas.
*   **Tests E2E con Playwright**: Creado `quiz-flow.spec.ts` bajo la carpeta `tests/` que valida el renderizado, launcher de exámenes y metadatos visuales del simulador.
*   **Fase 4 Refactoring (Limpieza de Código Muerto e Imports Barrel)**:
    - Eliminado el componente sin uso `UserProfileWidget.tsx` (que retornaba `null`) y borrada la carpeta vacía `components/common/`.
    - Creados archivos index barrel en `src/hooks/index.ts` (unificando 6 hooks) y en `src/lib/auth/index.ts` (`ensureAdminOrProfessor`, `assertAccess` y tipos).
    - Actualizados imports de 12 archivos clave (incluyendo `QuizInterface.tsx`, 7 Server Actions, 3 páginas de administración, `tenant-context.ts` y `tenants/route.ts`) para usar los nuevos barrels, resolviendo de forma limpia sin regresiones (compilación TypeScript satisfactoria).

### Sesión 49: 6 de Julio de 2026 (Reorganización de Navbar e Internacionalización del Menu en ABDQuiz)

*   **Foco en la Visión del Alumno**: Reordenada la barra de navegación lateral en `SidebarNavigation.tsx` colocando los menús de cara al estudiante (`Dashboard` ➔ `Exams` ➔ `My History`) en posiciones prioritarias.
*   **Ocultación de Home Pública**: Configurada la carga condicional para que la sección "Welcome Page" se oculte automáticamente ante sesiones de usuario logueadas (`isLoggedIn = true`), disminuyendo el clutter visual.
*   **Desambiguación Administrativa**: Renombrados los accesos de gestión añadiendo el prefijo `Admin:` (en inglés) y `Gestión:` (en español) en los diccionarios locales de `ABDi18n` para diferenciar con claridad las tareas estudiantiles de las operativas.
*   **Corrección de Analíticas**: El enlace a `/admin/dashboard` (que erróneamente se llamaba "Billing / Facturación") ha sido corregido a `Admin: Analytics & Metrics` / `Gestión: Analíticas`.
*   **Compilación y Validación de Tipos**: Las traducciones se compilaron exitosamente con `pnpm --filter @ajabadia/i18n build` y la app de Next.js pasa el typecheck (`tsc --noEmit`) con 0 errores.

### Sesión 50: 6 de Julio de 2026 (Auditoría de Seguridad e i18n en ABDAnalytics)

*   **Wrapper de Acciones del Servidor**: Creado `src/lib/actions-wrapper.ts` en `abd-analytics` con `withReadAction` que implementa validación de sesión de usuario y control de acceso basado en atributos (ABAC).
*   **Refactorización de getDashboardMetrics**: Migrada la Server Action central del panel de control para que consuma `withReadAction` y valide la capacidad `analytics:dashboard:view`, previniendo llamadas directas y fugas de datos de telemetría a nivel de endpoint.
*   **Resolución de Traducciones**: Agregadas las 9 claves de traducción de la capa de KPIs (`backToHome`, `activeStudents`, `completionRate`, `totalStorage`, `suiteHealth`, `generalServiceState`, `microservicePort`, `activeLicensedApps`, `failedLogins24h`) en los diccionarios inglés y español en `ABDi18n`, eliminando las advertencias y errores `MISSING_MESSAGE` en consola.
*   **Reorganización de la Navbar**: Reordenado `SidebarNavigation.tsx` para situar el panel de analíticas (`/admin`) en primer término, seguido de la auditoría de logs, y ocultar el acceso "Welcome Page" (`/`) ante sesiones autenticadas para limpiar redundancias del menú lateral, empleando traducciones específicas desacopladas en el namespace `analytics`.
*   **Unificación en el resto de Satélites**: Aplicados los mismos criterios de usabilidad de Navbar (priorizar dashboard y ocultar condicionalmente enlace de bienvenida para sesiones autenticadas) en `ABDtenantGovernance`, `ABDFiles`, `ABDLogs`, y en la plantilla de inicio `ABD___BASE`. Toda la suite pasa typecheck TypeScript libre de errores.
*   **Redirección de Aterrizaje Optimizada en ABDQuiz**: Cambiado el flujo de entrada de la aplicación `ABDQuiz`. Ahora, al hacer login o pinchar la aplicación desde el Portal Central, el alumno aterriza directamente en su **Student Dashboard** (`/dashboard`) en vez del listado crudo de exámenes (`/examinar`), proveyendo una bienvenida mucho más lógica y visualmente premium.
*   **Logs en Tiempo Real en ABDLogs**: Implementado el streaming en vivo de logs de auditoría vía Server-Sent Events (SSE) con limpieza de recursos asíncrona (`request.signal`) y un bus de eventos singleton en `src/lib/log-event-bus.ts`, e integrado el componente `LiveLogsConsole` con controles de pausa/limpieza y accesibilidad (`aria-label`) completa.
*   **Perfil Cognitivo en ABDQuiz**: Incorporado el panel `CognitiveStatusSection` que clasifica automáticamente los objetivos curriculares en Fortalezas (mastered, verde) y Áreas de Refuerzo (debilidades, ámbar) en la página principal del estudiante, auto-expandiendo además el primer curso por defecto para mejorar la experiencia de usuario.
*   **Robustez de Auditoría SOC2 en ABDLogs**: Creado el sistema de monitoreo en caliente de anomalías (`ingestion-guard.ts`) que vigila ráfagas sospechosas de autenticaciones fallidas (IP/60s) y registra alertas críticas. Implementado el endpoint de descarga de reportes PDF firmados con un hash de integridad SHA-256 acumulado (`report/route.ts`) junto a su interfaz en la consola de administración (`Soc2ReportDownload.tsx`), plenamente internacionalizado y con accesibilidad `aria-label` de botones validada.
...
*   [x] **5.2 Integración con Event Bus**: Migración de notificaciones síncronas hacia arquitectura orientada a eventos via Redis Streams (connector-listener.ts + EventBusBridge).
*   [x] **5.3 Observabilidad y Monitoreo SOC2**: Endpoint `GET /api/v1/health` con status, timestamp, uptime y service name. Patrón de ABDLogs replicado.
*   [x] **5.4 Motor de Búsqueda Integrado (MongoDB Atlas Search)**: Endpoint `GET /api/v1/documents/search?q=...` con pipeline `$search` sobre `title` y `tags`, filtrado por `tenantId`, paginación y fuzzy matching. Se elige Atlas Search sobre Elasticsearch/OpenSearch por no requerir infraestructura adicional.
*   [x] **5.5 Integración de ABAC Completo**: Enlace completo de todas las operaciones REST a nivel de fila y permisos con el `GuardianEngine` (abac.ts vía `evaluateAccess` del SDK).

### Fase 6: Operaciones Avanzadas e Integridad SOC2 (Futuro)
*   [x] **6.1 Despliegues de Alta Disponibilidad**: Blue-Green y configuraciones avanzadas de Vercel/PM2.
*   [x] **6.2 Cifrado Criptográfico a Nivel de Campo (MongoDB)**: Encriptación selectiva de campos sensibles del documento en base de datos.
*   [x] **6.3 Control y Versionado de Eventos (Event Sourcing)**: Esquema de versionado para el historial de transacciones de storage.
...
### Fase 8: Visualizaciones de Negocio y Robustez Criptográfica (Futuro / Brainstorming)
*   [ ] **8.1 Embudo de Conversión de Onboarding en ABDAnalytics**: Diseñar un widget visual de funnel (gráfico de embudo) para monitorizar el flujo de entrada de clientes (visitas ➔ envíos ➔ éxitos vs. errores).
*   [ ] **8.2 Suite de Coherencia Criptográfica en SDK**: Implementar pruebas automatizadas de regresión para garantizar que la derivación del IV determinista en `SecurityService` no sufra cambios a lo largo del tiempo.
*   [ ] **8.3 Sandbox GDPR Multi-Satélite**: Crear un script integrado que valide de extremo a extremo la purga atómica de usuarios simulados cruzando bases de datos de Quiz, Files y Logs.
*   [ ] **8.4 Chatbot de Analíticas Conversacionales (AI-Driven)**: Crear un asistente conversacional que permita a administradores consultar métricas mediante lenguaje natural, traduciéndolo a agregaciones dinámicas de MongoDB en `ABDAnalytics`.
*   [ ] **8.5 PWA y Repaso Offline-First en ABDQuiz**: Habilitar IndexedDB local para descargar exámenes y sincronizar estadísticas cognitivas del alumno al recuperar conexión.
*   [ ] **8.6 Búsqueda Semántica con Vector Search & Reuso de ABDAgRAG**: Integrar el indexador jerárquico de `ABDAgRAG` (`HierarchicalIndexer.ts` para segmentar documentos complejos en secciones y perfiles estructurados) antes de generar e indexar embeddings vectoriales en `ABDFiles` para búsquedas semánticas.
*   [ ] **8.7 Simulador de Marca Interactivo en Tiempo Real**: Crear un previewer en `ABDtenantGovernance` (usando *iframe templates*) que permita a los administradores previsualizar los cambios de contraste HSL/YIQ en la suite antes de aplicarlos.
