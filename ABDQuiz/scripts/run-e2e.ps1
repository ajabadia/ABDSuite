# run-e2e.ps1 — Run E2E tests for ABDQuiz on Windows
#
...
Write-Host "=== Step 2: Start ABDQuiz dev server on port $Port ==="
...
$Process = Start-Process node -ArgumentList "node_modules/next/dist/bin/next start -p $Port" -WorkingDirectory $ProjectRoot -NoNewWindow -PassThru -RedirectStandardOutput $LogFile -RedirectStandardError $ErrLogFile
...
try {
    ...
    for ($i = 1; $i -le 30; $i++) {
        try {
            ...
            # Even a redirect (302) or 404 is fine as long as the server responds
            ...
            # Invoke-WebRequest throws for non-200 responses under old PS version, check exception response
